...
| `mix tinfoil.build`      | Build a single target: run `mix release` with the right `BURRITO_TARGET`, package the binary into a `.tar.gz` (or `.zip` for Windows), and write a sha256 sidecar. Called by the generated workflow once per matrix entry. Checks the release tag against the mix.exs version first; the tag comes from `--tag` or `GITHUB_REF_NAME`, and `--skip-version-check` bypasses it. |
